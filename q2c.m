...
mkdir q2c_non_extrapolate;
mkdir q2c_extrapolate;
%parameters
epoch = 250;
...
for j = 1:length(no_hidden_neuron)
    ...
    name_scatter_valid = sprintf("q2c_non_extrapolate\\1-%d-1", no_hidden_neuron(j));
    name_scatter_test = sprintf("q2c_extrapolate\\1-%d-1", no_hidden_neuron(j));
    %plotting
    %validation data
...
function [net, tr] = train_bat(x_train, x_train_output, no_hidden_neuron, epoch, n)
    net = fitnet(no_hidden_neuron, 'trainbr');
